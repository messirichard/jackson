                    ...
                    <div style="max-width:600px;display:block;border-collapse:collapse;margin:0 auto;padding:30px 15px;border:1px solid #e7e7e7">
                        <table bgcolor="transparent" style="max-width:100%;border-collapse:collapse;border-spacing:0;width:100%;background-color:transparent;margin:0;padding:0">
                            <tbody>
                                <tr style="margin:0;padding:0">
                                    <td style="margin:0;padding:0">
                                        <h5 style="line-height:24px;color:#000;font-weight:900;font-size:17px;margin:0 0 20px;padding:0"><?php echo Yii::t('mail', 'Pesanan anda telah kami terima, kami akan menghubungi anda untuk biaya pengiriman dan total transfer') ?></h5>
                                        <?php /*
                                        ...
                                        <p style="font-weight:normal;font-size:14px;line-height:1.6;margin:0 0 20px;padding:0">Anda dapat memantau status pesanan dengan mengakses halaman Status Pemesanan pada akun <span class="lG"><?php echo Yii::app()->name; ?></span> Anda.</p>

                                        <div align="center" style="text-align:center;margin:20px;padding:0"><a target="_blank" style="margin:0;padding:7px 17px;color:#fff;text-decoration:none;display:inline-block;margin-bottom:0;vertical-align:middle;line-height:20px;font-size:13px;font-weight:600;text-align:center;white-space:nowrap;border-radius:2px;background-color:#777777;background-image:linear-gradient(top,#A2A2A2 0%,#575757 100%);border:1px solid #000000" href="<?php echo $url.CHtml::normalizeUrl(array('/member/vieworder', 'nota'=>$modelOrder->invoice_prefix.'-'.$modelOrder->invoice_no)); ?>">Lihat Pesanan Anda</a>
                                        </div>
                                        */ ?>

                                        <p style="font-weight:normal;font-size:14px;line-height:1.6;margin:0 0 20px;padding:0"><?php echo Yii::t('mail', 'Kami akan kembali memberikan informasi kepada Anda jika ada perubahan status pada pesanan anda.') ?></p>
                                        ...
                                        <h5 style="font-family:'HelveticaNeue-Light','Helvetica Neue Light','Helvetica Neue',Helvetica,Arial,'Lucida Grande',sans-serif;line-height:1.1;color:#000;font-weight:900;font-size:17px;margin:0 0 20px;padding:0"><?php echo Yii::t('mail', 'Rincian Pesanan:') ?></h5>
                                            ...
                                            <div style="margin:0;padding:0">
                                                ...
                                                <ol style="font-weight:normal;font-size:14px;line-height:1.6;margin:0 0 20px;padding:0 0 0 21px">
                                                    <li style="font-size:13px;margin:0 0 15px;padding:0">
                                                        ...
                                                        <br style="margin:0;padding:0">Grind: <?php echo $dataSerialize['grind'] ?>
                                                        <?php endif ?>
                                                        <?php if ($dataSerialize['garansi'] != ''): ?>
                                                        <br style="margin:0;padding:0">Garansi: <?php echo $dataSerialize['garansi'][1] ?>
                                                        <?php endif ?>
                                                        <br style="margin:0;padding:0"><?php echo Yii::t('mail', 'Jumlah') ?>: <?php echo $value['qty'] ?> <?php echo Yii::t('mail', 'Buah') ?> (<?php echo Cart::money($subTotal = ($value['total'])) ?>)</li>
                                                </ol>
                                                ...
                                                <div style="font-size:13px;line-height:18px;margin:0 0 15px;padding:0">
                                                    ...
                                                    <div style="margin:0 0 5px;padding:0"><b style="margin:0;padding:0"><?php echo Yii::t('mail', 'Tujuan Pengiriman:') ?></b>
                                                    ...
                                                    <br style="margin:0;padding:0">
                                                    <?php //echo City::model()->find('id = :id GROUP BY id', array(':id'=>$modelOrder->shipping_city))->type ?>
                                                    <?php echo $modelOrder->shipping_city;//echo City::model()->find('id = :id GROUP BY id', array(':id'=>$modelOrder->shipping_city))->city_name ?>,
                                                    <?php echo $model->shipping_zone;//echo City::model()->find('province_id = :province_id GROUP BY province_id', array(':province_id'=>$model->shipping_zone))->province ?>
                                                    <?php echo $model->shipping_postcode;//echo City::model()->find('province_id = :province_id GROUP BY province_id', array(':province_id'=>$model->shipping_zone))->province ?>
                                                    <br style="margin:0;padding:0">Telp: <?php echo $modelOrder->phone ?>
                                                    <br style="margin:0;padding:0">Email: <a href="mailto:<?php echo $modelOrder->email ?>"><?php echo $modelOrder->email ?></a>
                                                    <br style="margin:0;padding:0">Comment: <?php echo nl2br($modelOrder->comment) ?>
                                                </div>
                                                ...
                                                <div style="margin:20px 0 0;padding:0">
                                                    ...
                                                    */ ?>
                                                    ...
                                                    <?php if ($modelOrder->promo_id != 0): ?>
                                                    <div style="border-bottom-width:1px;border-bottom-color:#eee;border-bottom-style:solid;margin:0;padding:0">
                                                    </div>
                                                    <table bgcolor="transparent" style="width:100%;max-width:100%;border-collapse:collapse;border-spacing:0;background-color:transparent;margin:5px 0;padding:0">
                                                        <tbody style="margin:0;padding:0">
                                                            <tr style="margin:0;padding:0">
                                                                <td valign="top" style="width:50%;font-weight:700;font-size:13px;vertical-align:top;line-height:18px;margin:0;padding:0 10px 0 0">Diskon (<?php echo $modelOrder->promo_kode ?>):
                                                                </td>
                                                                <td align="right" valign="top" style="text-align:right;font-size:13px;vertical-align:top;line-height:18px;margin:0;padding:0 10px 0 0"><?php echo Cart::money($modelOrder->discount_total, 0) ?>
                                                                </td>
                                                            </tr>
                                                        </tbody>
                                                    </table>
                                                    ...
                                                    */ ?>
                                                    ...
                                                    <table bgcolor="transparent" style="width:100%;max-width:100%;border-collapse:collapse;border-spacing:0;background-color:transparent;margin:5px 0;padding:0">
                                                        <tbody style="margin:0;padding:0">
                                                            <tr style="margin:0;padding:0">
                                                                <td valign="top" style="width:50%;font-weight:700;font-size:13px;vertical-align:top;line-height:18px;margin:0;padding:0 10px 0 0"><?php echo Yii::t('mail', 'Total Pembayaran:') ?>
                                                                </td>
                                        ...
                                        <p style="margin-bottom:20px;font-weight:normal;font-size:14px;line-height:1.6;margin:40px 0 0 0;padding:10px 0 0 0;border-top:3px solid #d0d0d0"><small style="color:#999">
                                        <!-- Pantau status pemesanan Anda pada halaman <a target="_blank" style="margin:0;padding:0;color:#0f990f;text-decoration:none" href="<?php echo $url.CHtml::normalizeUrl(array('/member/index')); ?>">Status Pemesanan</a>. -->
                                            <!-- <br style="margin:0;padding:0"> -->
                                            <?php echo Yii::t('mail', 'Email ini dibuat secara otomatis. Mohon tidak mengirimkan balasan ke email ini.') ?></small></p>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                                ...
                                <tr style="margin:0;padding:0">
                                    <td valign="top" style="margin:0;padding:0 10px 0 0;width:75%">
                                        ...
                                        <span style="font-size:12px;margin-bottom:6px;display:inline-block">
                                            <b><?php echo Yii::t('mail', 'Hubungi') ?> <span class="lG"><?php echo Yii::app()->name; ?></span></b> <br>
                                            Phone: <?php echo $this->setting['contact_phone'] ?> <br>
                                            Email: <a href="mailto:<?php echo $this->setting['email'] ?>"><?php echo $this->setting['email'] ?></a> <br>
                                        </span> 
                                        <?php /*
