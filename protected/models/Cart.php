...
class Cart extends CFormModel
{
	 ...
	 * Add Cart
	 */
	public function addCart($id, $qty, $price, $option = '', $optional = '', $status = 'add')
	{
	...
	public function viewCart($language_id)
	{
		...
		foreach ($cart as $key => $value) {
			...
			$data[$key] = array(
				...
				'data'=>unserialize($dataProduct->data),
			);
		}
		}
		return $data;
			...
			foreach ($cart as $key => $value) {
	            if ($value['option'] != '') {
                    ...
                    if ($login_member['type'] == 'member') {
                        $value['price'] = $option->price - (0.1 * $option->price);
                    }
	            }
	...
	public static function increaseNota()
	{
		$date = date('Ymd');
		$dateSetting = Setting::model()->find('name = :name', array(':name'=>'invoice_date'));
		$numberSetting = Setting::model()->find('name = :name', array(':name'=>'invoice_number'));
		if ($date != $dateSetting->value) {
			$dateSetting->value = $date;
			$dateSetting->save(false);
			$numberSetting->value = 1000;
			$numberSetting->save(false);
		}else{
			$numberSetting->value = $numberSetting->value + 1;
			$numberSetting->save(false);
		}
		$nota = substr('0000'.$numberSetting->value, -4);
		return $nota;
	}
}