...
class PrdCategory extends CActiveRecord
{
	...
	public function relations()
	{
		...
		return array(
			...
			'parent'=>array(self::BELONGS_TO, 'PrdCategory', 'parent_id'),
			'child'=>array(self::HAS_MANY, 'PrdCategory', 'parent_id'),
			'product'=>array(self::HAS_ONE, 'PrdCategoryProduct', 'category_id'),
		);
	}
	...
	private $_nestedData;
	public function nested($data, $urlUpdate = 'category')
	{
		foreach ($data as $key => $value) {
			$this->_nestedData[$value->parent_id][$value->id] = $value->attributes;
			$this->_nestedData[$value->parent_id][$value->id]['name']=$value->name;
		}
		return $this->buildNested(0, $urlUpdate);
	}

	public function buildNested($parent_id = 0, $urlUpdate = 'category')
	{
        // $data=array();
        $str = '';
        if (count($this->_nestedData[$parent_id]) > 0) {
	        ...
	        foreach($this->_nestedData[$parent_id] as $key=>$val){            
		        $str .= '<li class="dd-item dd3-item" data-id="'.$val['id'].'">
	                ...
	                <a href="'.CHtml::normalizeUrl(array('/admin/'.$urlUpdate.'/delete', 'id'=>$val['id'], 'type'=>$val['type'])).'" class="delete"><i class="fa fa-trash-o"></i></a>
	                &nbsp;
	                <a href="'.CHtml::normalizeUrl(array('/admin/'.$urlUpdate.'/update', 'id'=>$val['id'], 'type'=>$val['type'])).'" class="update"><i class="fa fa-pencil"></i></a>
	                </div>
	            ';
	            $str .= $this->buildNested($key, $urlUpdate);
	        	$str .= '</li>';
	...
	public static function typeList($idProduct, $languageID)
	{
		$criteria = new CDbCriteria;
		$criteria->select = 't.id, description.*';
		$criteria->with = array('description', 'product');
		$criteria->addCondition('description.language_id = :language_id');
		$criteria->params[':language_id'] = $languageID;
		$criteria->addCondition('t.type = "filtercat"');
		$criteria->addInCondition('product.product_id', $idProduct);
		$criteria->order = 't.sort ASC';
		$criteria->group = 't.parent_id';
		$parent = PrdCategory::model()->findAll($criteria);
		$listData = array();
		foreach ($parent as $key => $value) {
			$listData[] = $value->parent_id;
		}
			$criteria = new CDbCriteria;
			...
			$criteria->params[':language_id'] = $languageID;
			$criteria->addCondition('t.type = "filtercat"');
			$criteria->addInCondition('t.id', $listData);
			$criteria->order = 't.sort ASC';
			$criteria->group = 't.id';
			$dataParent = PrdCategory::model()->findAll($criteria);
		...
		foreach ($dataParent as $key => $value) {
			...
			$criteria->order = 't.sort ASC';
			$criteria->group = 't.id';
			$child = PrdCategory::model()->findAll($criteria);
